...
exports.getProductsAvailable = () => {
    return new Promise((resolve, reject) => {
        ...
        db.all(sql, [], (err, rows) => {
            ...
            const products = rows.map((p) => ({id: p.id, name: p.name, farmerid: p.farmerid, price: p.price, measure: p.measure, category: p.category, typeofproduction: p.typeofproduction, picture: p.picture, dateavailability: p.dateavailability, quantity: p.quantity}))
